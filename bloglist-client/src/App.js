...
function App() {
    ...
    const handleLikeChange = async (id, blog) => {
        ...
        await blogService.updateBlog(id, updateBlog).then((data) => {
            const updatedBlog = blogPosts.map((blog) => (blog.id === id ? { ...blog, likes: data.likes } : blog));
    ...
    const addBlog = (blogObject) => {
        blogService
            ...
            .then((data) => {
                dispatch(setNotification(["success", `${data.title} has been added successfully`]));
                setTimeout(() => {
                    dispatch(setNotification([]));
                }, 5000);
    ...
    const handleDelete = (id, title) => {
        ...
        if (action) {
            ...
            blogService
                .deleteBlog(id)
                .then(() => {
                    dispatch(setNotification(["success", `${title} has been deleted successfully`]));
                    setTimeout(() => {
                        dispatch(setNotification([]));
                    }, 5000);
                    const filterblog = blogPosts.filter((blog) => blog.id !== id);
    ...
    return (
        <Router>
            ...
            <Routes>
                ...
                <Route
                    path="/users"
                    element={<Users users={users} currentUser={signedInUser.username} handleLogout={handleLogout} />}
                />
                <Route
                    path="/users/:userId"
                    element={<User users={users} signedInUser={signedInUser} handleLogout={handleLogout} />}
                />
            </Routes>
        </Router>
    );
